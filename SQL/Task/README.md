...
Create a table named "employees" with columns (emp_id, emp_name, department, salary) and insert data.
...
Create a table named "orders" with columns (order_id, customer_id, total_amount) and insert data.
...
Create two tables named "students" and "courses" with columns. Write an SQL query to retrieve the student name, course name, and credits for all enrolled courses.
...
Create three tables named "employees," "departments," and "salaries" with columns. Write an SQL query to retrieve the department name and the average salary of employees working in each department. Sort the results by the average salary in descending order.
